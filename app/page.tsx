...
export default function Home() {
  const [activeSample, setActiveSample] = useState("vulnerable");
  const [source, setSource] = useState(SAMPLE_BY_ID["vulnerable"].source);
    ...
    <div className="space-y-10">
      <section className="space-y-4">
        ...
        <p className="max-w-2xl text-sm leading-relaxed text-[var(--ink-muted)]">
          Paste an Anchor / Rust program (or load a sample). Token2022 Guard runs {CHECKS.length} checks for the
          extension bugs auditors keep finding: transfer-hook guards, re-entrancy, unvalidated extra accounts,
          fee math, permanent delegates, confidential balances, and account sizing.
        </p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="space-y-5">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`badge ${highCrit > 0 ? "sev-critical" : "badge-pass"}`}
            >
          ...
          {result.findings.length === 0 ? (
            <div className="panel">
              <div className="panel-inner text-sm text-[var(--ink-muted)]">
                No issues detected by the current checks. {source.trim().length === 0 ? "Paste a program or load a sample to begin." : "Looks clean — still get a professional audit before mainnet."}
              </div>
            </div>
          ) : (
      ...
      {/* Checks catalog */}
      <section className="space-y-4">
        <h2 className="label">Check catalog</h2>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {CHECKS.map((c) => {
            const fired = result.findings.some((f) => f.checkId === c.id);
            return (
              <div key={c.id} className={`panel sev-${c.severity}`}>
                <div className="panel-inner space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`badge ${fired ? `sev-${c.severity}` : "badge-pass"}`}>
                      {fired ? c.severity : "pass"}
                    </span>
                  </div>
